"""Phase 2 (possession-adjusted): which forwards fit Flick's Barcelona best.
...
Output: output/forward_fit_adj.csv
"""
import numpy as np
import pandas as pd
...
ADJ = [c + '_adj' for c in SUMS]
SUMS = SUMS + [c for c in ADJ if c in m.columns]
...
for base in ['ss_shotsFromInsideTheBox', 'us_np_xg', 'ss_possessionWonAttThird',
             'ss_ballRecovery', 'ss_tackles', 'ss_wasFouled', 'ss_dispossessed',
             'us_xa', 'us_xg_chain', 'ss_keyPasses', 'ss_bigChancesCreated',
             'ss_successfulDribbles']:
    if base + '_adj' in p.columns:
        p[base + '_padj90'] = p[base + '_adj'] / n
...
DIMS = {
    'FINISH': [('npxg_per_shot', .45), ('ss_goalConversionPercentage', .35),
               ('fifa_finishing', .20)],
    'BOX':    [('ss_shotsFromInsideTheBox_padj90', .45), ('us_np_xg_padj90', .35),
               ('box_share', .20)],
    'PRESS':  [('ss_possessionWonAttThird_padj90', .55),
               ('ss_ballRecovery_padj90', .25), ('ss_tackles_padj90', .20)],
    'HOLDUP': [('ss_wasFouled_padj90', .35), ('ss_aerialDuelsWonPercentage', .25),
               ('ss_groundDuelsWonPercentage', .25), ('ss_dispossessed_padj90', -.15)],
    'LINK':   [('us_xa_padj90', .30), ('us_xg_chain_padj90', .25),
               ('ss_keyPasses_padj90', .20), ('ss_bigChancesCreated_padj90', .15),
               ('ss_accuratePassesPercentage', .10)],
    'RUN':    [('ss_topSpeed', .40), ('ss_successfulDribbles_padj90', .35),
               ('sprints_p90', .25)],
}
...
cand.to_csv('output/forward_fit_adj.csv', index=False, encoding='utf-8')
